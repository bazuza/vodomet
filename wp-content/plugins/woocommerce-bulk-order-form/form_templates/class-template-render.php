...
class WooCommerce_Bulk_Order_Form_Template_UI {
	...
	public function get_product_column( $count, $echo = true ) {
		...
		<select name="wcbulkorder[wcbof_products][' . $count . '][product_name]"  class="product_name product_name_search_field" placeholder="'.$placeholder.'" style="'.$select_style.'"></select>';
	...
	public function get_variation_column( $count, $echo = true ) {
		$return = '<div class="attributes" id="attribute_' . $count . '"  data-count="' . $count . '">
			<input data-count="' . $count . '" type="hidden" name="wcbulkorder[wcbof_products][' . $count . '][variation_name]" value="" class="variation_name"/>
		</div>';
