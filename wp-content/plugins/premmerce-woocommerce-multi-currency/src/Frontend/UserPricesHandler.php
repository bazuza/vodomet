...
class UserPricesHandler
{
    ...
    {
        add_filter('get_post_metadata', [$this, 'replacePrice'], 10, 4);


        //Manage Woocommerce cached prices
        add_filter('woocommerce_get_variation_prices_hash', [$this, 'manageWoocommercePricesHash'], 10, 3);
        ...
        if(isset($_GET['min_price']) || isset($_GET['max_price'])){
            ...
            add_action('plugins_loaded', function(){
                ...
                if(version_compare(wc()->version, '3.6', '>=')){
                    add_filter('posts_clauses', [$this, 'fixProductQueryWithPriceFilter'], 9, 2);
                }else{
        ...
        add_filter('woocommerce_get_price_html', [$this, 'addPriceSpan'], 100, 2);

        if ($this->ajaxPricesEnabled) {
            add_action('wp_footer', [$this, 'outputCurrencyIdSpan'], 999);
        ...
        add_action('plugins_loaded', function(){
            if(has_filter('premmerce_multicurrency_get_price_for_user')){
                ...
                foreach($priceFilters as $filter){
                    add_filter($filter, [$this, 'applyGetPriceForUserFilter'], 100, 2);
                }
            }
    ...
    public function replacePrice($originalValue, $objectId, $metaKey, $single){
        ...
        if(in_array($postType, ['product', 'product_variation'])){
            remove_filter(current_filter(), [$this, 'replacePrice']);
            ...
            add_filter(current_filter(), [$this, 'replacePrice'], 10, 4);
        }
    ...
    private function replacePriceFieldsInArray($metaData, $objectId){
        ...
        foreach ($metaData as $key => $value){
            if (array_key_exists($key, $this->fieldsToReplace)) {
                $currencyId = $this->getProductCurrencyByProductId($objectId);
                $fieldsToReplace = $this->getFieldsToReplace($objectId);
                $metaData[$key] = [$this->getPriceForProductField($objectId, $fieldsToReplace[$key] , $key, $currencyId)];
            }
        }
    ...
    public function calculatePriceInUsersCurrency($price, $priceCurrencyId = '', $convertToCurrencyId = '')
    {
        ...
        return (string) apply_filters('premmerce_multicurrency_price_in_users_currency', $price, $priceCurrencyId,
            $convertToCurrencyId);
    }
    ...
    public function sendPricesForAjax()
    {
        ...
        foreach ($filterData as $key => $value) {
            ...
            $value = in_array($key, ['min', 'minRange']) ? floor($value) : ceil($value);
    ...
    public function fixProductQueryWithPriceFilter(array $args, WP_Query $query){
        if(has_filter('posts_clauses', [wc()->query, 'price_filter_post_clauses'])){
