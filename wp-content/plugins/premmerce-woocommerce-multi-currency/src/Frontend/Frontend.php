...
class Frontend
{
    ...
    public function __construct(FileManager $fileManager, Model $model, UserCurrencyHandler $userCurrencyHandler)
    {
        ...
        add_shortcode('multicurrency', [$this, 'registerMulticurrencyShortcode']);

        add_action('wp_enqueue_scripts', [$this, 'enqueueAssets']);

        if(isset($_GET['currency_id'])){
            add_action('woocommerce_cart_loaded_from_session', [$this, 'updateCartTotals']);
        }
    ...
    {
        wp_enqueue_script('premmerce-multicurrency-selector',
        ...
        if (get_option('premmerce_multicurrency_ajax_prices_redraw')) {
            ...
            wp_enqueue_script('premmerce-multicurrency-ajax-prices',
                $this->fileManager->locateAsset('frontend/js/premmerce-multicurrency-ajax-prices.js'),
                ['jquery-ui-slider', 'wc-jquery-ui-touchpunch', 'accounting'],
                '3.3.3'
            );
        ...
        wp_enqueue_style('multicurrency_widget_style',
    ...
    public function registerMulticurrencyShortcode()
    {
        ...
        return $this->fileManager->renderTemplate('frontend/currency-selector.php',
            ['currencies' => $addedCurrencies, 'usersCurrency' => $this->userCurrencyId]);
    }
