...
class Model
{
    ...
    {
        return apply_filters('premmerce_multicurrency_geolocation_countries_and_currencies',
    ...
    {
        return (bool)$this->wpdb->get_var($this->wpdb->prepare("SELECT EXISTS(SELECT id FROM {$this->currenciesTable} WHERE id = '%d')",
            $id));
    }
    ...
    public function addCronSchedule($interval = 0)
    {
        ...
        add_filter('cron_schedules', function ($schedules) use ($interval) {
            $schedules[self::RATES_UPDATER_INTERVAL_NAME] = [
                'interval' => $interval,
    ...
    private function updateExistingCurrency($currencyData){
        ...
        if ($id == $this->getMainCurrencyId() && 1 != $currencyData['rate']) {
            ...
            if (!$currencyData['display_on_front'] && $this->isLastAvailableForUsersCurrency($id)) {
                ...
                $result->setMessage(
                    __('You are trying to set this currency unavailable for users, but this is last available currency. At least one currency must be available.',
    ...
    public function setOrderCurrencyId(WC_Order $order, $userCurrencyId)
    {
        ...
        $order->save();
        do_action('premmerce_multicurrency_order_currency_id_updated', $order->get_id(), $userCurrencyId,
    ...
    private function saveDefaultCurrencyCountries($currencyId)
    {
        ...
        switch ($_POST['currency-countries']) {
            case 'include':
                $countriesList = array_map('esc_sql', $_POST['currency-countries-list']);
                $type = 'include';
                break;
    ...
    private function addProductMetaFields(WC_Product $product)
    {
        ...
        $addedFields = ['currency' => $currencyId];
    ...
    public function recalculateProductPrices(WC_Product $product)
    {
        ...
        $productNewRegularPrice = round($productCurrencyRegularPrice * $productCurrencyRate,
    ...
    public function fillMainCurrencyData()
    {
        ...
        $format = ['%s', '%s', '%f', '%s', '%s', '%s', '%s', '%d'];
        $mainCurrencyId = (int)get_option(self::MAIN_CURRENCY_OPTION_NAME);
        $this->wpdb->update($this->currenciesTable, $data, ['id' => $mainCurrencyId], $format);
    }
    ...
    public function updateDB()
    {
        ...
        if (!$this->getCurrencies()) {
            ...
            $this->wpdb->insert($this->currenciesTable, $defaultCurrency,
                ['%s', '%s', '%f', '%s', '%s', '%s', '%s', '%d']);
    ...
    {
        wp_cache_delete('currencies_front');
        wp_cache_delete('currencies_all');
    }
    ...
    public function deleteProductsCurrenciesMetaFields($onlyForCurrency = null)
    {
        ...
        $fieldsListProductVariation = ['_product_currency_regular_price', '_product_currency_sale_price'];
        $fieldsListProduct = array_merge($fieldsListProductVariation, ['_product_currency']);

        $onlyForCurrency = apply_filters('premmerce_multicurrency_delete_product_meta_fields_currency',
            $onlyForCurrency);
    ...
    public function deleteCurrency($currencyId)
    {
        ...
        if ($this->isLastAvailableForUsersCurrency($currencyId)) {
            if ($this->getCurrencyById($currencyId)['display_on_front']) {
                $this->wpdb->update($this->currenciesTable, ['display_on_front' => 1], ['id' => $mainCurrencyId], '%d',
                    '%d');
            }
        }
        ...
        $result = $this->wpdb->delete($this->currenciesTable, ['id' => $currencyId], '%d');
