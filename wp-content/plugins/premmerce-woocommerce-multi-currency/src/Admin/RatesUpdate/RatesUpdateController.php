...
class RatesUpdateController
{
     ...
     */
    private $availableRatesUpdaters = [];
    ...
    public function __construct(Model $model)
    {
        ...
        add_action('plugins_loaded', [$this, 'loadUpdaters']);

        add_action('shutdown', [$this, 'saveUpdatersData']);
    }
    ...
    public function getRates($currencies)
    {
        ...
        return ['data' => $ratesByIds, 'message' => $message];
    }
    ...
    public function getCurrencyRate($currencyCode, $updaterId)
    {
        ...
        if ($updater = $this->getUpdaterById($updaterId)) {
            $rates = $updater->getRates([$currencyCode]);
    ...
    public function saveUpdaterOptions()
    {
        ...
        if ($updaterInstance) {
            ...
            $result['message'] = __('You are trying to save updaters options, but such updater not found. Data wasn\'t saved.',
    ...
    private function prepareResultMessage($currenciesUpdateFailed)
    {
        if ($failedCount = count($currenciesUpdateFailed)) {
            ...
            $message['message'] = sprintf(__("Can not find updater selected for currencies {$placeholders} . Please, set another updater on currency edit page or fix chosen to update this currency exchange rate."),
