...
class CurrenciesListTable extends \WP_List_Table
{
     ...
     * @return string
     */
    function column_currency_name($item)
    {

        $deleteUrl = wp_nonce_url(sprintf('%s?action=%s&currency_id=%d', admin_url('admin-post.php'), 'delete-currency',
            $item['id']), 'premmerce-currency-delete');
    ...
    function column_default($item, $column_name)
    {
        ...
        switch ($column_name) {
            ...
            case 'example':
                ...
                add_filter('woocommerce_currency_symbol', [$this, 'changeCurrencySymbol']);
                $this->currentCurrencySymbol = $item['symbol'];
                $price = wc_price(1000.00, $priceFormatArgs);
                remove_filter('woocommerce_currency_symbol', [$this, 'changeCurrencySymbol']);

                return '<span class="price-example">' . $price . '</span>';
            case 'display_on_front':
                $messageUsersCant = __('Users can\'t choose this currency on site.',
            ...
            default:
                return apply_filters('premmerce_multicurrency_list_table_column_' . $column_name . '_content',
                    $item[$column_name]);
        }
    }
    ...
    function prepare_items()
    {
        ...
        foreach ($currencies as &$currency) {
            $currency['isMain'] = '<input class="is-main-checkbox" name="is-main" type="radio" value="' . $currency['id'] . '"' . checked(get_option(Model::MAIN_CURRENCY_OPTION_NAME),
                    $currency['id'], false) . ' >';
            $currency['rate'] = rtrim(rtrim(sprintf('%.10F', $currency['rate']), '0'), ".");
        }

        usort($currencies, [&$this, 'sortItems']);
