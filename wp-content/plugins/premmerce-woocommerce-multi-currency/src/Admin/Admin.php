...
class Admin
{
    ...
    public function subscribe()
    {
        ...
        add_action('admin_init', [$this->pluginPages, 'registerSettings']);
        add_action('admin_menu', [$this->pluginPages, 'addMenuItem']);
        add_filter('woocommerce_get_settings_general', [$this->pluginPages, 'moveWoocommerceOptions']);
        add_action('admin_enqueue_scripts', [$this->pluginPages, 'enqueueAdminAssets']);

        //Show warning if main currency was changed without this plugin
        add_action('admin_init', [$this, 'checkCurrencySettings']);
        ...
        add_action('wp_ajax_' . self::CHECK_UPDATERS_STATUSES_ACTION, [$this, 'checkUpdatersStatuses']);


        //Recalculate, update, change, delete
        add_action('admin_post_update-currencies', [$this, 'insertCurrencyData']);
        add_action('admin_post_add_currency', [$this, 'insertCurrencyData']);
        add_action('admin_post_delete-currency', [$this, 'deleteCurrency']);
        add_action('admin_post_premmerce_multicurrency_update_rates', [$this, 'updateRatesButtonHandler']);
        add_action('admin_post_' . self::CLEAN_PLUGIN_CACHE_ACTION, [$this, 'cleanPluginCache']);
        add_action('wp_ajax_' . self::DISMISS_UPDATER_MESSAGE_ACTION, [$this, 'dismissUpdaterNoApiKeyMessage']);
        add_action('wp_ajax_premmerceChangeShopCurrency', [$this, 'changeShopCurrency']);
        add_action('wp_ajax_premmerceRecalculatePrices', [$this, 'updateProductsDataAJAX']);


        //Save rates updaters settings
        add_action('admin_post_save_multicurrency_updater_settings', [$this, 'saveRatesUpdaterOptions']);
    }
    ...
    public function updateProductsDataAJAX($onlyForCurrency = '')
    {
        ...
        $productsToUpdateData = get_transient('premmerce_products_recalculate') ?: [];
    ...
    public function deleteCurrency()
    {
        ...
        $currencyDeleteNotAllowedMessage = [
            'message' => __('Currency wasn\'t deleted. Looks like you are not permitted to do this.',
                'premmerce-woocommerce-multicurrency'),
            'type' => 'error'
        ];

        $currencyDeleteFailMessage = [
            'message' => __('Something goes wrong. Currency wasn\'t deleted.'),
            'type' => 'error'
        ];

        $currencyDeleteSuccessMessage = [
    ...
    public function insertCurrencyData()
    {
        ...
        if ('update-currencies' === $_POST['action']) {
            ...
            $adminNotice['message'] .= '<br><a href="' . $this->defaultRedirectPath . '">' . __('Back to the currencies list',
    ...
    public function saveRatesUpdaterOptions()
    {
        ...
        current_user_can('manage_woocommerce') || $this->redirectBack([$notPermittedMessage]);
    ...
    public function checkCurrencySettings()
    {
        ...
        if ($pluginMainCurrency['code'] && $woocommerceMainCurrency && $woocommerceMainCurrency !== $pluginMainCurrency['code']) {
            $this->notifier->push(__('Looks like main store currency was changed since Premmerce Multi-currency was active last time. Please, check your prices and currencies rates and update it, if needed.',
                'premmerce-woocommerce-multicurrency'), 'warning');
        }
     ...
     *
     */
    private function redirectBack($notice = [], $path = '')
    {
        if ($notice) {
            $this->notifier->flash($notice['message'], $notice['type'] ?: 'success');
        }
