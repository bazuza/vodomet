...
class PluginPages
{
    ...
    {
        register_setting( 'multicurrency_settings', 'premmerce_multicurrency_ajax_prices_redraw', [
            'type'              => 'boolean',
        ...
        add_settings_field(
            ...
            $this->fileManager->includeTemplate( 'admin/ajax-prices-redraw-checkbox.php', [
                'val' => $val,
            ] );
        },
            Admin::PAGE_SLUG,
    ...
    public function enqueueAdminAssets()
    {
        ...
        if ( stristr( get_current_screen()->id, 'premmerce_multicurrency' ) ) {
            ...
            //jQuery validate
            wp_enqueue_script( 'validate-plugin', '//cdn.jsdelivr.net/npm/jquery-validation@1.17.0/dist/jquery.validate.min.js', [ 'jquery' ] );
            $lang = ( 'uk' === get_locale() ? 'uk' : substr( get_locale(), 0, strpos( get_locale(), '_' ) ) );
            if ( $lang != 'en' ) {
                //try to load validate plugin translation
                wp_enqueue_script( 'validate-localization-' . $lang, '//cdn.jsdelivr.net/npm/jquery-validation@1.17.0/dist/localization/messages_' . $lang . '.js', [ 'validate-plugin' ] );
            }
            wp_localize_script( 'validate-plugin', 'premmerceMulticurrencyData', [
                'language' => $lang,
            ] );
            //Select2
            wp_enqueue_script(
    ...
    private function renderCurrencyEditPage( $updatersList, $mainCurrencyArray )
    {
        ...
        $currencyData['countries'] = ( isset( get_option( Model::DEFAULT_COUNTRY_CURRENCY_OPTION_NAME )[$currencyID] ) ? get_option( Model::DEFAULT_COUNTRY_CURRENCY_OPTION_NAME )[$currencyID]['countries'] : [] );
        ...
        $formFields = $this->fileManager->renderTemplate( 'admin/currency-form-fields.php', [
            'page'             => 'edit-currency',
        ...
        $this->fileManager->includeTemplate( 'admin/edit-currency-page.php', [
    ...
    private function renderEditUpdaterPage()
    {
        ...
        $this->fileManager->includeTemplate( 'admin/rates-updater-settings-page.php', [
    ...
    private function renderCurrenciesTablePage( $updaters, $mainCurrencyData )
    {
        ...
        $this->fileManager->includeTemplate( 'admin/currencies-page/currencies-page.php', [
            'formFields'      => $formFields,
