...
use Premmerce\WoocommerceMulticurrency\Widget;
...
class WoocommerceMulticurrencyPlugin implements PluginInterface
{
    ...
    public function run()
    {
        ...
        if ($this->woocommerceActive) {
            ...
            add_action('wc_ajax_premmerce_get_prices', [$this, 'handleAJAXrequest']);
            add_action('wc_ajax_nopriv_premmerce_get_prices', [$this, 'handleAJAXrequest']);
            $this->model->addCronSchedule();

            add_action('woocommerce_loaded', [$this, 'load']);//to be sure woocommerce is already loaded and we can use it's classes, functions etc.
            ...
            if (is_admin()) {
                add_action('admin_init', function () {
                    if (current_user_can('activate_plugins')) {
                        ...
                        $notifier->push(__('Premmerce Woocommerce Multicurrency requires <a href="https://wordpress.org/plugins/woocommerce/">WooCommerce</a> to be activated to work.',
                            'premmerce-woocommerce-multicurrency'), 'error', true);
                    }
                });
    ...
    public function load(){
        ...
        add_action(Model::RATES_UPDATER_CRON_HOOK, [$this->ratesUpdateController, 'updateRates']);
